...
<div class="app-card mb-4">
  <div class="app-card-body">
    <div class="d-flex align-items-center gap-4">
      <!-- Avatar / Photo -->
      <div id="profileImageContainer" title="Click to upload profile image" style="position:relative;width:80px;height:80px;border-radius:50%;background:var(--primary);display:flex;align-items:center;justify-content:center;font-size:2rem;font-weight:800;color:#fff;overflow:hidden;cursor:pointer;flex-shrink:0;border:3px solid var(--primary-border);">
      ...
      <div class="flex-grow-1">
        <h2 class="page-title mb-0"><?= htmlspecialchars($user['name']) ?></h2>
        <p class="mb-2" style="font-size:13px;color:var(--muted);"><?= htmlspecialchars($user['email']) ?></p>
        <!-- Completion bar -->
        <?php
...
<div class="row g-4">
  ...
  <div class="col-lg-5">
    <div class="app-card h-100">
      <div class="app-card-header">
        <h2 class="app-card-title"><i class="bi bi-people-fill text-primary"></i> Linked Teachers</h2>
      </div>
      <div class="app-card-body">
        <p class="mb-3" style="font-size:13px;color:var(--muted);">Add or remove the teachers linked to your account.</p>
        <form method="POST" id="teachersForm">
          ...
          <div id="linked_teachers_list" class="d-flex flex-column gap-2 mb-3">
            <?php foreach ($linkedTeachers as $lt): ?>
              <div class="d-flex align-items-center gap-3 p-2" style="background:var(--surface-subtle);border-radius:var(--radius);border:1px solid var(--border);">
                <div class="sidebar-avatar" style="flex-shrink:0;"><?= strtoupper(substr($lt['name'], 0, 1)) ?></div>
                <div class="flex-grow-1" style="font-size:13.5px;color:var(--on-surface);font-weight:500;"><?= htmlspecialchars($lt['name']) ?></div>
                <input type="hidden" name="teacher_ids[]" value="<?= (int)$lt['teacher_id'] ?>">
                <button type="button" class="btn btn-sm btn-outline-danger remove-linked" style="flex-shrink:0;"><i class="bi bi-x"></i></button>
              </div>
            <?php endforeach; ?>
          </div>
          <div id="add_teacher_row_container"></div>
          <div class="d-flex gap-2">
            <button type="button" id="addTeacherBtn" class="btn btn-outline-primary btn-sm"><i class="bi bi-plus"></i> Add Teacher</button>
            <button type="submit" name="update_teachers" class="btn btn-primary btn-sm"><i class="bi bi-check-circle"></i> Save</button>
          </div>
        </form>
      </div>
    </div>
  ...
  <!-- Profile Form -->
  <div class="col-lg-7" id="profileForm">
    <div class="app-card">
      <div class="app-card-header">
      ...
      <div class="app-card-body">
        <form method="POST">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6">
              <label class="form-label">NIC No <span style="font-weight:400;color:var(--muted);">(if available)</span></label>
              <input type="text" name="nic_no" class="form-control" value="<?= htmlspecialchars($profile['nic_no'] ?? '') ?>">
            </div>
            <div class="col-md-6">
              <label class="form-label">Postal ID Card No <span style="font-weight:400;color:var(--muted);">(if available)</span></label>
              <input type="text" name="postal_id_card_no" class="form-control" value="<?= htmlspecialchars($profile['postal_id_card_no'] ?? '') ?>">
            </div>
          </div>

          <hr class="my-4" style="border-color:var(--border);">
          <h6 class="fw-bold mb-3" style="font-size:13px;text-transform:uppercase;letter-spacing:0.06em;color:var(--muted);"><i class="bi bi-building me-2"></i>School Details</h6>
