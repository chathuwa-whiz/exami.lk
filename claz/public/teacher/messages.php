...
if ($msgTableExists) {
  ...
  $contactStmt = $pdo->prepare(
    'SELECT s.id, s.name, s.email, 0 AS unread
     FROM teacher_student ts
     JOIN users s ON s.id = ts.student_id
     WHERE ts.teacher_id = ?
     GROUP BY s.id, s.name, s.email
...
$activePeerId = isset($_GET['student_id']) ? (int)$_GET['student_id'] : 0;
    ...
    <div class="app-card h-100 d-flex flex-column" style="overflow:hidden;">
      <div class="app-card-header">
        <div>
          ...
          <h2 class="app-card-title mt-0">Students</h2>
        </div>
      ...
      <div class="flex-grow-1" style="overflow-y:auto;">
        <?php if (empty($contacts)): ?>
          <div class="p-4 text-center">
            ...
            <p class="mb-0" style="font-size:13px;color:var(--muted);">No students linked yet.</p>
          </div>
        <?php else: ?>
    ...
    <div class="app-card h-100 d-flex flex-column" style="overflow:hidden;">
      ...
      <div class="app-card-header flex-shrink-0">
        <div>
          ...
          <h2 class="app-card-title mt-0" id="peerName">Select a student to start</h2>
        </div>
      ...
      <div class="flex-grow-1 chat-scroll" id="chatMessages" role="log" aria-live="polite" aria-busy="false" style="overflow-y:auto;padding:16px;">
        <p class="text-center mb-0 py-4" style="color:var(--muted);">Select a student to load messages.</p>
      </div>
...
(function(){
    ...
    list.forEach(msg => {
      ...
      wrapper.innerHTML = `
        <div class="chat-bubble">
          ${messageHtml}
          ${attachmentHtml}
          <div class="chat-meta">${isSent ? '📤 You' : '📥 Student'} · ${humanTime(msg.created_at)}</div>
        </div>`;
  ...
  function sendMessage(evt){
    evt.preventDefault();
    if (!activePeerId) { showAlert('Select a student first.'); return; }
