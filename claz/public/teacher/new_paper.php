...
<section style="background: white; padding: 0; margin-bottom: 2rem;">
  <div style="padding: 2rem; max-width: 900px; margin: 0 auto;">
    ...
    <form method="post" id="paperForm" accept-charset="UTF-8" enctype="multipart/form-data">
      ...
      <div style="background: #f8fbff; border: 2px dashed #a3bced; padding: 1.5rem; text-align: center; border-radius: 8px; margin-bottom: 2rem;">
        <h4 style="font-size: 1.1rem; color: #4b5fc0; margin-bottom: 0.5rem;"><i class="bi bi-file-earmark-pdf"></i> Auto-Generate from PDF</h4>
        <p style="font-size: 0.9rem; color: #666; margin-bottom: 1rem;">ප්‍රශ්න සහ පිළිතුරු ඇතුලත් PDF එකක් අප්ලෝඩ් කරන්න. සිස්ටම් එක ස්වයංක්‍රීයව ප්‍රශ්න ටික හඳුනාගෙන ෆෝම් එකට ඇතුලත් කරාවි.</p>

        <div style="display: flex; justify-content: center; gap: 1rem; align-items: center; max-width: 500px; margin: 0 auto;">
          <input type="file" id="pdfImportFile" accept="application/pdf" class="form-control">
          <button type="button" class="btn btn-primary d-inline-flex align-items-center gap-2" onclick="processPDF()" id="pdfImportBtn">
            <i class="bi bi-magic"></i> Extract Q&A
          </button>
        </div>

        <div id="pdfLoading" style="display: none; margin-top: 1rem; color: #0078d4; font-weight: 500;">
          <span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>
          AI එක මගින් ප්‍රශ්න වෙන් කරමින් පවතී... කරුණාකර රැඳී සිටින්න.
        </div>
      </div>
...
async function processPDF() {
  ...
  if (!fileInput.files || fileInput.files.length === 0) {
    alert('කරුණාකර පළමුව PDF ගොනුවක් තෝරන්න!');
    return;
  }
  ...
  try {
    const response = await fetch('<?= htmlspecialchars(app_href('api/extract_pdf.php'), ENT_QUOTES) ?>', {
      method: 'POST',
      body: formData
    });
    ...
    if (data.success && data.questions && data.questions.length > 0) {
      ...
      if (existingCards.length === 1) {
        const firstQText = existingCards[0].querySelector('.question-text').value;
        if (!firstQText || firstQText.trim() === '') {
      ...
      data.questions.forEach(q => {
        ...
        addQuestion(prefillData, 'choice');
      });

      alert(`සාර්ථකයි! ප්‍රශ්න ${data.questions.length} ක් ෆෝම් එකට ඇතුලත් කරන ලදී.`);
    } else {
      alert('අසාර්ථකයි: ' + (data.error || 'Unknown error occurred'));
    }
  } catch (error) {
    console.error('Extraction Error:', error);
    alert('සර්වර් එක හා සම්බන්ධ වීමේදී දෝෂයක් මතු විය. Console එක පරීක්ෂා කරන්න.');
  } finally {
    loadingDiv.style.display = 'none';
    btn.disabled = false;
    fileInput.value = ''; 
    debounceSaveDraft(); // අලුතින් ආපු ප්‍රශ්න ටික draft එකට සේව් කරන්න
  }
}
